...
1. `.specify/memory/constitution.md` — the project's fixed principles (Tier A)
2. `specs/adr/adr-index.md` → the ADRs it routes for the target area — check no ADR blocks the change (Tier A)
3. The project's rules: the `.claude/rules/{stack}/` folders mapped to `{Project}` by `rules.stacks` in
   `.specify/profile.yaml` (default Backend → `backend`, Frontend → `web`, Mobile → `mobile`) — the target
   state follows them (Tier A). A missing folder logs `.claude/rules/{stack}/ not present — skipped`.
...
- Current state: what smells / violations are present (list with file:line references; cite the rule,
  ADR or constitution principle each one breaks, where one applies)
- Target state: what patterns/abstractions replace them (per the project's rules, routed ADRs and loaded packs)
...
- No ADR, constitution principle or project rule violated
