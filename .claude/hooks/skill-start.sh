#!/usr/bin/env bash
# Skill-start hook — runs on BOTH entry paths of an sk.* skill:
#   PreToolUse(Skill)   the model invokes a skill (orchestrators call sub-skills this way)
#   UserPromptSubmit    the user types `/sk.x …`, which expands as a command with no Skill tool call
# 1. Evaluates the skill's declared preconditions. Exit 2 = block (the prompt or tool call is refused).
# 2. On pass, records the start: active skill + role, the SK_RESULT marker, and the transitions known
#    at invocation time (sk_on_skill_start in lib-story.sh; governance/status-model.md).
#
...
INPUT=$(cat)
SKILL_NAME=$(sk_skill_from_input "$INPUT")
[[ -z "$SKILL_NAME" ]] && exit 0
...
start_and_exit() {
  sk_on_skill_start "$PROJECT_ROOT" "$SKILL_NAME"
  exit 0
}
...
eval_rule() {
  ...
  if [[ "$rule" == story.* ]]; then
    ...
    case "$result" in
      ...
      *)
        echo "skill-start.sh: WARNING — unrecognized rule: ${rule}" >&2 ;;
    esac
    return
  fi
...
while IFS= read -r RULE; do
  ...
  if [[ "$RULE" == when\ * ]] && [[ "$RULE" == *" => "* ]]; then
    ...
    case "$(story_condition "$COND")" in
      ...
      *)       echo "skill-start.sh: WARNING — unrecognized condition: ${COND}" >&2 ;;
    esac
    continue
  fi
