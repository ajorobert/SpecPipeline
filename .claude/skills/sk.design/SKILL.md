---
name: sk.design
description: "Invoke when: running the full design pipeline for a unit in one shot. Role: architect (orchestrator). Invokes with the Skill tool: sk.design_sub_architecture → [review gate] → sk.design_sub_datamodel → [review gate] → sk.design_sub_contracts → [review gate] → sk.design_sub_ui-design (frontend units). Reads: .specify/state/session.yaml, unit-brief.md, 01-story/, specs/adr/adr-index.md, specs/domain/bounded-contexts.md, .specify/memory/projects/index.md. Writes: unit guide.yaml (sub-skills write 02-design/)."
...
each with the Skill tool, then sk.design_sub_ui-design when the unit has a user-facing surface.
