...
## Input Artifacts
.specify/state/session.yaml (active focus for context)
...
2. Determine feature name from active_unit_id or active_story_id in `.specify/state/session.yaml`
   (ask the user when neither is set)
3. Run `bash {SCRIPTS_DIR}/create-phr.sh "{feature-name}"` — it creates `history/prompts/{feature}/` on
   first use and prints the reserved file path
4. Write PHR into that file using `{TEMPLATES_DIR}/artifacts/phr-template.md`
