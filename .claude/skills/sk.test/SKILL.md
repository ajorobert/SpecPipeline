---
name: sk.test
description: "Invoke when: generating and running the test suite for a unit, producing one test folder per impacted project. Role: lead (orchestrator). Runs at unit level. Dispatches with the Agent tool, one at a time: sk.test_sub_testproject (once per impacted project), as the QA role for that project type. Produces 05-test/{Project}/ — Backend: unit-test.md, integration-test.md, contract-test.md; Frontend/Mobile: component-test.md, contract-test.md. Reads: .specify/state/session.yaml, .specify/profile.yaml (contracts.verify), unit-brief.md, knowledge-base.md, 02-design/contract-changes.md, the canonical specs/openapi|asyncapi files it lists, 01-story acceptance criteria, .specify/memory/projects/{Project}/tech-stack.md."
...
Dispatches `sk.test_sub_testproject` with the Agent tool for each impacted project (from the unit's
